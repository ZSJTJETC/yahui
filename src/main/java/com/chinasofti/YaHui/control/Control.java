...
public class Control {
	...
	private void removeEmployee() {
		while(true){
			...
			System.out.println("员工id\t员工姓名\t员工密码\t员工性别\t员工权限\t员工地址");
	...
	private void settleAccounts() {
		...
		if(s.equals("y")){
			while(true){
				...
				if(ifCard==true){
					System.out.println("应付:"+d+"元");
					Card card = service.ifCard(i);
					System.out.println("卡内余额为:"+card.getCmoney()+"元");
					if(card.getCmoney()>=d){
						...
						System.out.print("实收："+sumMoney*cDiscount);
						double Ymoney=sumMoney*cDiscount;
						System.out.println("卡内余额："+(card.getCmoney()-sumMoney*cDiscount));
		...
		if(jin<d){
			...
			System.out.println("找零:"+(jin-sumMoney));
			System.out.println("=============谢谢惠顾============");
			this.Employee();
		}
	}
	...
	private void openCard() {
		int select=this.ui.getInt("请输入充值金额:");
		System.out.println("您得到的卡号为:");
		int select2=this.ui.getInt("请设置密码:");
		int select3=this.ui.getInt("请再次输入密码:");
		//判断密码是否一致
		System.out.println("开卡成功！");
		System.out.println("您已成为XXX会员");
		System.out.println("卡号为XXXX");
		System.out.println("余额为XXX元");
		System.out.println("0、返回");
		int select4=this.ui.getInt("请选择：");
		if(select4==0){
			this.v.println("返回");
	...
	private void reportCard() {
		int select=this.ui.getInt("请输入用户id:");
		int select1=this.ui.getInt("请输入要挂失的卡号:");
		//判断用户和卡号是否是绑定的
		System.out.println("尊敬的XXX，您的卡号XXXX已成功挂失");
		System.out.println("0、返回");
		int select4=this.ui.getInt("请选择：");
		if(select4==0){
			this.v.println("返回");
	...
	private void reissueCard() {
		...
		int select2=this.ui.getInt("请设置新会员卡的密码:");
		int select3=this.ui.getInt("请再次输入密码:");
		//判断密码是否一致
		System.out.println("尊敬的XXX，您补办成功！");
		System.out.println("卡号为XXXX");
		System.out.println("余额为XXX元");
		System.out.println("0、返回");
		int select4=this.ui.getInt("请选择：");
	...
	private void rechargeCard() {
		System.out.println("特大喜讯！每月28号充值200返50！！！");
		this.showCard();
		int select2=this.ui.getInt("请输入要充值金额:");
		System.out.println("充值成功");
		System.out.println("用户XXX您的卡号为:XXX");
		System.out.println("余额:XXX元");
		System.out.println("0、返回");
		int select4=this.ui.getInt("请选择：");
		if(select4==0){
			this.v.println("返回");
	...
	private void balanceCard() {
		...
		int select3=this.ui.getInt("请输入卡密码:");
		//展示卡的信息
		System.out.println(" ");
	...
	//修改员工的个人信息
		private void modifyEmployee() {
			//可以修改姓名，性别，卡密码
			while(true){
			int select=this.ui.getInt("请输入要修改卡号:");
			int select2=this.ui.getInt("请输入修改后的姓名:");
			int select3=this.ui.getInt("请输入修改后的性别:");
			int select4=this.ui.getInt("请输入修改后的密码:");
			System.out.println("0、返回");
			int select5=this.ui.getInt("请选择：");
			if(select5==0){
				this.v.println("返回");
